---
title: "05. Projet Jeu de Société : Ludopédagogie & Édumédias"
description: "Hub du Projet Fil Rouge : Ludopédagogie, règles, cartes IA, FabLab, capsule vidéo et playtest"
---

<script setup>
const subCategories = [
  {
    title: "5.1 Concept & Ludopédagogie",
    tag: "Théorie du Jeu",
    desc: "Pourquoi créer un jeu pour enseigner ? De Huizinga à Caillois, le cercle magique et l'apprentissage expérientiel.",
    link: "/modules/05-projet-jeu-societe#5-1-une-methodologie-pedagogique-fondee-sur-l-experience-ludique",
    image: "/images/tile-5-projet-jeu.jpg"
  },
  {
    title: "5.2 Règles & Mécaniques",
    tag: "Game Design",
    desc: "Concevoir les boucles de jeu, équilibrer les chances, rédiger un livret de règles clair et sans ambiguïté.",
    link: "/modules/06-regles-du-jeu",
    image: "/images/image36.png"
  },
  {
    title: "5.3 Photographie & Graphisme",
    tag: "Identité Visuelle",
    desc: "Cadrage, éclairage, droit à l'image des élèves, retouche et mise en page sur Canva.",
    link: "/modules/07-photographie-image",
    image: "/images/image38.png"
  },
  {
    title: "5.4 [Jalon] Cartes & IA",
    tag: "Jalon Noté 1",
    desc: "Générer les illustrations des cartes de jeu via Midjourney/DALL-E, prompts rigoureux et gabarits d'impression.",
    link: "/modules/08-cartes-ia",
    image: "/images/image40.png"
  },
  {
    title: "5.5 [Jalon] FabLab (Laser/3D)",
    tag: "Jalon Noté 2",
    desc: "Fabrication numérique au FabLab : vectorisation pour découpe laser du plateau et impression 3D des pions.",
    link: "/modules/09-prototypage-fablab",
    image: "/images/image42.png"
  },
  {
    title: "5.6 Capsule Vidéo Promo",
    tag: "Communication",
    desc: "Scénarisation, tournage face caméra, montage dynamique et pitch de présentation de votre jeu.",
    link: "/modules/10-capsule-video",
    image: "/images/image44.png"
  },
  {
    title: "5.7 [Jalon] Playtest & Retours",
    tag: "Évaluation Finale",
    desc: "Tester le jeu en conditions réelles avec des élèves, recueil de feedbacks et ajustements itératifs.",
    link: "/modules/11-playtest-evaluation",
    image: "/images/image26.jpg"
  }
]
</script>

# 05. Projet Jeu de Société : Ludopédagogie & Édumédias

::: info Le Projet Fil Rouge du Quadrimestre
Durant ce cours, vous allez concevoir en équipe un **jeu de société physique ludo-éducatif** centré sur une thématique d'éducation aux médias (infox, cyberharcèlement, identité numérique, droit à l'image, bulles de filtres).
Ce projet mobilise le numérique pour la création graphique (IA, Canva), la fabrication matérielle (FabLab HECh) et la communication audiovisuelle (capsule vidéo).
:::

### 🎯 Parcourez les jalons et étapes du projet :

<SubCategoryTiles :items="subCategories" />

---

## 5.1. Une méthodologie pédagogique fondée sur l’expérience ludique

La ludopédagogie désigne une approche pédagogique qui mobilise le jeu comme moyen de construire des apprentissages. Elle ne consiste donc pas simplement à « rendre un cours amusant » ou à ajouter une activité ludique à une séquence traditionnelle. Pour qu'il y ait véritablement ludopédagogie, le jeu doit participer de manière cohérente à l'atteinte des objectifs d'apprentissage.

L'apprenant est placé en situation d'action, de décision, d'expérimentation, de coopération et de résolution de problèmes. Il apprend en faisant, mais également en analysant ce qu'il fait et en confrontant ses productions à celles des autres.

Consultez les différents jalons ci-dessus pour préparer chaque étape de conception de votre jeu.
